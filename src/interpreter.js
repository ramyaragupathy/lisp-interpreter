const interpret = (inp) => {
  ...
  mathExpParser(inp) || symbolParser(inp) || whiteSpaceParser(inp)
}
...
input[0] === '<') ? [input[0], input.slice(1)] : null
}
const env = {
  '>=': (operands) => operands.reduce((a, b) => { return a >= b ? '#t' : '#f' }),
  '<=': (operands) => operands.reduce((a, b) => { return a <= b ? '#t' : '#f' }),
